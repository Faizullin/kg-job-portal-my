...
const NEW_JOBS_QUERY_KEY = 'new-jobs';

export function NewJobsTab() {
  const newJobsQuery = useQuery({
    queryKey: [NEW_JOBS_QUERY_KEY],
    queryFn: async () => {
      const response = await myApi.v1HomeMasterNewJobsList({
        ordering: '-created_at',
        pageSize: 20,
      });
  ...
  const newJobs = newJobsQuery.data?.results || [];

  if (newJobsQuery.isLoading) {
    return (
        ...
        <Card key={job.id} className="hover:shadow-md transition-shadow border border-gray-200">
          <CardContent className="p-4">
            ...
            <Button 
              ...
              variant={job.status === Status30eEnum.published ? 'default' : 'outline'}
            >
              Подробнее
