...
const loadApplicationsQueryKey = 'applications';
...
const RenderTable = () => {
  ...
  const loadApplicationsQuery = useQuery({
    queryKey: [loadApplicationsQueryKey, debouncedSearchQuery],
    queryFn: async () => {
  ...
  useEffect(() => {
    if (!loadApplicationsQuery.data) return;
    const parsed = loadApplicationsQuery.data.results || [];
    setParsedData(parsed);
    setParsedPagination({
      pageCount: Math.ceil(
        (loadApplicationsQuery.data.count || 0) / 10,
      ),
    });
  }, [loadApplicationsQuery.data]);
  ...
  const withdrawMutation = useMutation({
    mutationFn: async (applicationId: number) => {
      const response = await myApi.v1ApplicationsWithdraw({
        id: applicationId,
        jobApplicationRequest: {
          applicant: null,
          status: "withdrawn" as JobApplicationStatusEnum,
        }
      });
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [loadApplicationsQueryKey] });
  ...
  const handleWithdraw = useCallback((application: JobApplication) => {
  ...
  const columns = useMemo<ColumnDef<JobApplication, any>[]>(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const canStart = application.status === "accepted";
  ...
  ], [getStatusColor, getStatusIcon, formatDate, formatBudget, handleWithdraw]);
  ...
  if (loadApplicationsQuery.isLoading) {
    return (
  ...
  if (loadApplicationsQuery.error) {
    return (
      <div className="text-center py-8">
        <p className="text-red-600">Error loading applications</p>
        <Button onClick={() => loadApplicationsQuery.refetch()} className="mt-2">
          Try Again
        </Button>
      </div>
    );
