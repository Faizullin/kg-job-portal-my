...
interface OrderCreateEditDialogProps {
  control: ReturnType<typeof useDialogControl<OrderFormData>>;
  onSave?: (data: OrderFormData) => void;
  onCancel?: () => void;
}

export function OrderCreateEditDialog({
  control,
  onSave,
  onCancel
}: OrderCreateEditDialogProps) {
  ...
  const orderMutation = useMutation({
    mutationFn: async (formData: OrderFormData) => {
      ...
      // Find subcategory ID if subcategory is selected
      ...
      if (isEdit && data?.id) {
        ...
        return myApi.v1OrdersUpdate({ id: data.id, orderUpdate: orderData }).then((r: any) => r.data);
      } else {
        // Create new order
        return myApi.v1OrdersCreateCreate({ orderCreate: orderData }).then((r: any) => r.data);
      }
    },
    onSuccess: () => {
  ...
  return (
    <FormDialog
      ...
      submitText={isEdit ? 'Update Order' : 'Create Order'}
      maxWidth="3xl"
